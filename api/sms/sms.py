...
def send_sms(api_key: str, api_secret: str, sender: str, addressee: str, text: str) -> dict:
  ...
  try:
    ...
    if not sender or not addressee or not text:
      return {
        "status": "ko",
        "message": "Sender, addresse, and text message are required"
      }
    ...
    if not re.match(r'^[a-zA-Z0-9\s]+$', sender):
      return {
      "status": "ko",
      "message": "Sender must be an alphanumeric string (spaces allowed)"
      }
    ...
    if not re.match(phone_pattern, addressee):
      return {
        "status": "ko",
        "message": "Addressee must be a valid phone number"
      }
