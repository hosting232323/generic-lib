...
def log_request(response, log_folder, excluded_paths, excluded_prefixes, user_log_field=None):
  try:
    if is_loggable(excluded_paths, excluded_prefixes):
      body = extract_response(response)
      if body is not None:
        write_log(g.get('log_user'), log_folder, body, swagger=g.get('log_swagger', False), user_field=user_log_field)
  except Exception:
    traceback.print_exc()
  return response
