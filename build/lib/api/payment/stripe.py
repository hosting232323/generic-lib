...
def create_stripe_session_(params: PaymentRequest):
  try:
    ...
    if params.mode == 'subscription':
      session_params['subscription_data'] = {'metadata': validated_metadata}
...
def stripe_webhook(payload, sig_header, webhook_secret):
  ...
  try:
    ...
    if event['type'] not in ['invoice.paid', 'invoice.payment_succeeded', 'checkout.session.completed']:
      return {'status': 'ko', 'message': 'Event type not handled'}

    return {'status': 'ok', 'event': event['type'], 'metadata': event['data']['object'].to_dict()}
...
def get_customer_data(filters):
  try:
    ...
    if not customers:
      return {'status': 'ko', 'message': 'Customer not found'}

    # if more than one customer is found, return an error
    if len(customers) > 1:
      return {'status': 'ko', 'message': 'Multiple customers found with the same email'}
    ...
    if 'subscriptions' in filters.get('params', {}):
      ...
      for subscription in subscriptions:
        ...
        if (
          filters.get('metadata')
          and filters.get('metadata').get('projects')
          and project not in filters.get('metadata').get('projects')
        ):
          continue
    ...
    return {'status': 'ok', 'customer': customer_data}
  except stripe.error.StripeError as e:
    raise e
