...
def report_subscription_usage(stripe_api_key, customer_id, usage_value, event_name):
  try:
    ...
    return {'status': 'ok', 'meter_event': meter_event}
...
def delete_subscription(stripe_api_key, subscription_id):
  try:
    ...
    if deleted_subscription.status == 'canceled':
      return {'status': 'ok', 'message': 'Subscription successfully canceled', 'subscription': deleted_subscription}
    else:
      return {'status': 'ko', 'message': f'Failed to cancel subscription. Status: {deleted_subscription.status}'}
  except stripe.error.StripeError as e:
    raise e
  except Exception as e:
    raise e
